...
class ExperimentWriter:
    ...
    def save(self) -> None:
        ...
        if self._fs.isdir(self.log_dir) and not self._fs.isfile(hparams_file):
        ...
        if False:
            try:
                self._fs.put_file(tmp.name, self.metrics_file_path)
            finally:
                os.unlink(tmp.name)

        # with self._fs.open(file_uri, "w", newline="", s3={"profile": "writer"}) as f:
        #     writer = csv.DictWriter(f, fieldnames=metrics_keys)
        #     writer.writeheader()
        #     writer.writerows(self.metrics)


class XaminLogger(Logger):
    r"""
